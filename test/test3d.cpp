...
void TEST_1() {
    Point *p_points = new Point[(7 - 3) * 60];
    ...
    //gen_cr_line(p_points, p_tan_points, p_cpoints, 7, 60);
    for (int i = 0; i < (7 - 3) * 60; ++i) {
        printf("point:  %f %f %f\n", p_points[i].pos[0], p_points[i].pos[1],
               p_points[i].pos[2]);
        printf("direct: %f %f %f\n", p_tan_points[i].pos[0],
               p_tan_points[i].pos[1], p_tan_points[i].pos[2]);
    }
    Vector v1, v2, v;
    init_point(1.0, 0.0, 0.0, &v1);
    init_point(0.0, 1.0, 0.0, &v2);
    gen_rotate_vec(v1, v2, &v);
    printf("vector: %f %f %f\n", v.pos[0], v.pos[1], v.pos[2]);

    float mat[16];
    gen_quater_matrix(v, 20.0 * PI / 180.0, mat);
